...
public class Server {
    ...
    private static String DB_URL = "jdbc:sqlite:data\\contacte.db";
    ...
    private static void processServerConnection(Socket socket){
        try(
            ...
            while(true){
                ...
                System.out.println("Current thread: "+ Thread.currentThread().getId() + " received: " + payload);

                if("Add".equals(payload.message)){
                    synchronized (mutex) {
                        var contactToAdd = (Contact) payload.payload;
                        int uniqueCode = getContacts().stream()
                                        .map(Contact::getCode)
                                        .max(Integer::compareTo)
                                        .orElse(0)+1;
                        
                        addContact(new Contact(uniqueCode,
    ...
    public static void main(String[] args) {
        ...
        addContact(new Contact(100,"dasda","10202022"));
        System.out.println(getContact());*/
